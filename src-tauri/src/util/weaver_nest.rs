...
/// overrides: Map of asset_id -> Some(pack_id) for penciled assets
...
pub fn build_weaver_nest(
    ...
    pack_order: &[String],
    overrides: &HashMap<String, String>, // asset_id -> pack_id
    output_dir: &str,
) -> Result<()> {
    ...
    for asset in assets {
        let winner_pack_id = if let Some(override_pack_id) = overrides.get(&asset.id) {
            // Use override
            override_pack_id.clone()
        } else {
        ...
        // Find the file to copy (first one in the asset's file list)
        // In a real implementation, you might want to merge multiple files
        if let Some(source_file) = asset.files.first() {
